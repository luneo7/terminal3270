...
namespace Open3270.TN3270
{
	internal class Controller : IDisposable
	{
        ...
        String DumpRangeString(int first, int length, bool inAscii, byte[] buffer, int relRows, int relCols, bool hidden)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    linebuf.Append(Convert.ToString((byte)0));    
                    any = false;
                }
                if (!any)
                {
		...
		void DumpRangeXML(int first, int length, bool inAscii, byte[] buffer, int relRows, int relCols, bool hidden)
		{
        ...
        int GetScreenField(int address, ExtendedAttribute ea, Screen screen)
        {
            byte fa = this.fakeFA;
            int faIndex;
            int start, baddr;
            ...
            for (rowCount = rowStart; rowCount < rowEnd; rowCount++)
            {
                ...
                screenField.Location = new ScreenLocation();
                screenField.Location.position = start;
                screenField.Location.left = AddressToColumn(start);
                screenField.Location.top = AddresstoRow(start);
                screenField.Location.length = length;
                screenField.Attributes = new ScreenAttributes();
                

                bool hidden = false;

                screenField.Attributes.Base = fa;               

                if (FieldAttribute.IsProtected(fa))
                {
                    screenField.Attributes.Protected = true;
                }
              
                if (FieldAttribute.IsZero(fa))
                {
                    screenField.Attributes.FieldType = "Hidden";
                    hidden = true;
                }
                else if (FieldAttribute.IsHigh(fa))
                {
                    screenField.Attributes.FieldType = "High";
                }
                else if (FieldAttribute.IsIntense(fa))
                {
                    screenField.Attributes.FieldType = "Intense";
                }
                else
                {
                    if (ea.fg != 0)
                    {
                        screenField.Attributes.Foreground= See.GetEfaUnformatted(See.XA_FOREGROUND, ea.fg);
                    }
                    if (ea.bg != 0)
                    {
                        screenField.Attributes.Background= See.GetEfaUnformatted(See.XA_BACKGROUND, ea.bg);
                    }
                    if (ea.gr != 0)
                    {
                        screenField.Attributes.Highlighting= See.GetEfaUnformatted(See.XA_HIGHLIGHTING, (byte)(ea.bg | 0xf0));
                    }
                    if ((ea.cs & ExtendedAttribute.CS_MASK) != 0)
                    {
                        screenField.Attributes.Mask= See.GetEfaUnformatted(See.XA_CHARSET, (byte)((ea.cs & ExtendedAttribute.CS_MASK) | 0xf0));
                    }
                }

                screenField.Text = this.DumpRangeString(start, length, true, screenBuffer, rowCount, columnCount, hidden);
                if (String.IsNullOrWhiteSpace(screenField.Text))
                {
                    screenField.Text = null;
                }
		...
		int DumpFieldAsXML(int address, ExtendedAttribute ea)
		{
			...
			for (rowCount = rowStart; rowCount < rowEnd; rowCount++)
			{
				...
				this.DumpRangeXML(start, length, true, screenBuffer, rowCount, columnCount, hidden);
				this.telnet.Action.action_output("</Field>");
			}
		...
		public bool DumpXMLAction(params object[] args)
		{
			...
			telnet.Action.action_output("<?xml version=\"1.0\" encoding=\"iso-8859-1\"?>");// encoding=\"utf-16\"?>");
			...
			for (i = 0; i < rowCount; i++)
			{
				...
				this.DumpRangeXML(start, length, true, screenBuffer, rowCount, columnCount, false);
				this.telnet.Action.action_output("</Text>");
			}
			this.telnet.Action.action_output("</Unformatted>");
        ...
        public bool GetScreenAction(params object[] args)
        {
            int pos = 0;

            Screen screen = new Screen();
            screen.CX = columnCount;
            screen.CY = rowCount;
            screen.Formatted = this.isFormatted;
            screen.Fields = new System.Collections.Generic.List<ScreenField>();
            if (screen.Formatted)
            {               
                ExtendedAttribute ea = new ExtendedAttribute();
                // CFCJR Mar 4,2008 : user tmcquire in post on www.open3270.net
                ...
                {
                    lastPos = pos;
                    pos = GetScreenField(pos, ea, screen);
                    if (lastPos == pos)
                    {
                        cnt++;
                    }
            ...
            //Output unformatted image anyway
            int i;
            screen.Unformatted = new UnformattedScreen();
            screen.Unformatted.Text = new System.Collections.Generic.List<string>();
            for (i = 0; i < rowCount; i++)
            {
                int start = RowColumnToByteAddress(i, 0);

                int length = columnCount;
                screen.Unformatted.Text.Add(DumpRangeString(start, length, true, screenBuffer, rowCount, columnCount, false));
            }
            screen.Render();
            this.telnet.Action.action_output(screen);
            return true;
        }
