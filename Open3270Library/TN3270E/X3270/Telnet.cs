	...
	internal class Telnet : IDisposable
	{
			...
			get
			{
				...
				else
				{
					if (this.disconnectReason == null)
						this.disconnectReason = "O servidor fechou o socket de conexão";
					return false;
				}
			}
		}
		...
		public void Connect(object parameterObjectToSendCallbacks, string hostAddress, int hostPort)
		{
			...
			else
			{
				...
				if (!ipaddress)
				{
					...
					{
						throw new TNHostException("Não foi possível resolver host '" + address + "'", se.Message, null);
					}
				}
				else
				{
					...
					{
                        throw new TNHostException("Endereço TCP/IP '" + address + "' de host inválido", se.Message, null);
					}
				}
				...
				if (!string.IsNullOrEmpty(sourceIP))
				{
					...
					{
						throw new TNHostException("Endereço TCP/IP '" + address + "' de fonte inválida", se.Message, null);
					}
				}
				else
				{
				...
				{
					throw new TNHostException("Ocorreu um erro ao tentar conectar com o host '" + address + "' na porta " + port, se.Message, null);
				}
				...
				lock (this)
				{
					if (socketStream != null)
					{
						...
						if (string.IsNullOrEmpty(disconnectReason))
							this.disconnectReason = "foi solicitado o stream socket de desconexão telnet";
					}
					//
					if (socketBase != null)
					{
						...
						try
						{
							...
							if (string.IsNullOrEmpty(disconnectReason))
								this.disconnectReason = "base socket de desconexão telnet solicitado";
						}
		...
		public bool ParseByte(byte b)
		{
			lock (this)
			{
				...
				if (this.TelnetProcessFiniteStateMachine(ref sbBuffer, b) != 0)
				{
					...
					this.Disconnect();
					this.disconnectReason = "Erro na máquina de estados Telnet durante ParseByte";
					return false;
				}
			}
			return true;
		...
		private void ConnectCallback(IAsyncResult ar)
		{
			try
			{
				...
				{
					this.disconnectReason = "Não foi possível conectar ao host - erro de estouro de tempo para conexão";
			...
			catch (Exception ex)
			{
				...
				this.Disconnect();
				this.disconnectReason = "exceção durante o callback de conexão telnet";
			}
		}
		...
		private void OnRecievedData(IAsyncResult ar)
		{
			...
			if (socketBase == null || socketBase.Connected == false)
			{
				...
				if (string.IsNullOrEmpty(disconnectReason))
				{
					disconnectReason = "O host fechou a conexão ou não está conectado no telnet.OnReceivedData";
				}
			}
			else
			{
				try
				{
					...
					{
						disconnectme = true;
						this.disconnectReason = "Nenhum dado foi recebido no telnet.OnReceivedData, desconectando";
					}
				}
				catch (System.ObjectDisposedException)
				{
					disconnectme = true;
					disconnectReason = "Cliente fechou a conexão";
				}
				catch (Exception e)
				{
					...
					disconnectme = true;
					disconnectReason = "Exeção no stream de dados (" + e.Message + "). Conexão fechada.";
				}
			}
		...
		public bool WaitForConnect()
		{
			while (!IsAnsi && !Is3270)
			{
				...
				if (!IsResolving)
				{
					this.disconnectReason = "Estouro de limite de tempo na espera de conexão";
					return false;
				}
			}
			return true;
