	...
	internal class Actions
	{
		...
		internal Actions(Telnet tn)
		{
			...
			actions = new XtActionRec[] {
                    ...
                    new XtActionRec( "getscreen",	false,	new ActionDelegate(telnet.Controller.GetScreenAction )),
        ...
        public void action_output(object data)
        {
            dataobjectcapture = data;
        }
		...
		public void action_output(string data, bool encode)
		{
            ...
            datacapture.Add(System.Text.Encoding.GetEncoding("ISO-8859-1").GetBytes(data));
			//
			if (encode)
			{
				data = encodeXML(data);
		...
		public void action_output(byte[] data, int length, bool encode)
		{
			...
			string strdata = System.Text.Encoding.GetEncoding("ISO-8859-1").GetString(temp);
			if (encode)
			{
		...
		public bool Execute(bool submit, string name, params object[] args)
		{
			...
			{
				throw new Open3270.TNHostException("Host TN3270 não está conectado", telnet.DisconnectReason, null);
			}

			datacapture = null;
			datastringcapture = null;
            dataobjectcapture = null;
