...
namespace Open3270
{
	...
	public class TNEmulator : IDisposable
	{
		...
		IScreen currentScreenObject; // don't access me directly, use helper
        IXMLScreen currentScreenXML;
        ...
        public int KeyboardLocked
        {
            get
            {
                if (currentConnection == null) throw new TNHostException("Terminal não está conectado", "Não existe conexão com terminal 3270 aberta", null);
        ...
        /// </summary>
        /// 
        public IScreen CurrentScreenObject
        {
            get
            {
                ...
                if (this.currentScreenObject == null)
                {
                    ...
                    {
                        sout.WriteLine("CurrentScreenObject reloading by calling GetScreenAsObject()");
                        currentScreenObject = GetScreenAsObject();
        ...
        public IXMLScreen CurrentScreenXML
        {
            get
            {
                ...
                if (this.currentScreenXML == null)
                {
                    if (sout != null && Debug == true)
                    {
                        sout.WriteLine("CurrentScreenXML reloading by calling GetScreenAsObject()");
        ...
        public bool SendKey(bool waitForScreenToUpdate, TnKey key, int timeout)
        {
            ...
            if (triggerSubmit)
            {
                lock (this)
                {
                    this.DisposeOfCurrentScreenObject();
                    currentScreenObject = null;
                ...
                if (waitForValidScreen)
                {
                    ...
                    do
                    {
                        ...
                        if (timeout > 0)
                        {
                            ...
                            if (!IsConnected)                            
                            {
                                if (this.currentConnection != null && this.currentConnection.DisconnectReason != null)
                                    throw new TNHostException("Conexão TN3270 perdida", this.currentConnection.DisconnectReason, null);
                                else
                                    throw new TNHostException("Conexão TN3270 perdida", "sessão desconectada", null);
                            }
        ...
        {
            return this.CurrentScreenObject.GetText(x, y, length);
        }
        /// <summary>
        ...
        public void Connect(string host, int port, string lu)
        {
            ...
            try
            {
                ...
                //mScreenProcessor.Update_Screen(currentScreenObject, true);
        ...
        protected void DisposeOfCurrentScreenObject()
        {
            if (currentScreenObject != null)
            {
                IDisposable disposeXML = currentScreenObject as IDisposable;
                if (disposeXML != null)
                    disposeXML.Dispose();
                currentScreenObject = null;
            }
        ...
        protected virtual void Dispose(bool disposing)
        {
            lock (this)
            {
                ...
                if (disposing)
                {
                    ...
                    if (sout != null)
                        sout.WriteLine("TNEmulator.Dispose() Disposing of currentScreenObject");
        ...
        internal IScreen GetScreenAsObject()
        {
            DisposeOfCurrentScreenObject();

            if (currentConnection == null) throw new TNHostException("Terminal não está conectado", "Não existe conexão com terminal 3270 aberta", null);
            if (currentConnection.ExecuteAction(false, "GetScreen"))
            {
                //
                return (Screen)currentConnection.GetObjectData();
        ...
        private void currentConnection_RunScriptEvent(string where)
        {
            lock (this)
            {
                ...
                semaphore.Release(1);
                OnScreenContentChanged();
            }
        }


        public event EventHandler ScreenContentChanged;
        protected virtual void OnScreenContentChanged()
        {
            if (this.ScreenContentChanged != null)
            {
                this.ScreenContentChanged(this, EventArgs.Empty);
            }
        }
        /// <summary>
